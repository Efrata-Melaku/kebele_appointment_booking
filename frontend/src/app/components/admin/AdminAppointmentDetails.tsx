  ...
  if (error || !detail) {
    return (
      <div className="space-y-4">
        <Link to="/admin/appointments" className="text-sm text-blue-600 hover:underline inline-flex items-center gap-1">
          ...
          <ul className="space-y-2 text-sm">
            {groupHistory.map((h) => (
              <li key={h.id} className="flex flex-wrap gap-2 border-b border-gray-50 pb-2">
                ...
                {h.id !== appointment.id ? (
                  <Link to={`/admin/appointments/${h.id}`} className="text-blue-600 hover:underline">
                    View
                  </Link>
                ) : (
